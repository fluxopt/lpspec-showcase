...
markdown = math_spec.to_markdown(archives[0].read_text())
...
Printed from the `model.yaml` that the solve job archived, by the language's own typesetter. No data binds and no solver runs to produce this page: every equation below is what the YAML states, and every number on the other pages is what the solver made of it.
