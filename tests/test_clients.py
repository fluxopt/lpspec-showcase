...
def test_the_clients_page_prints_what_both_returned(runs: Path):
    """The page loader runs both clients at build time, so the numbers on it are not transcribed."""
    done = subprocess.run(
    ...
    assert '| polars | DuckDB | agree |' in page
    assert '**no**' not in page, 'every number on the page agrees between the two clients'
    assert page.count('| `base` |') == len(NUMBERS), 'one row per headline number per run'
    assert 'def headline(run: Path)' in page and 'read_parquet' in page, (
        'both sources are printed from the files that run'
    )
